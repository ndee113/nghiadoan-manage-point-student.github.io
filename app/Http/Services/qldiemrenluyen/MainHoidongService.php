...
class MainHoidongService
{
    ...
    public function get_diem_distinct_join($filters = []){
        $diem = Diem::select('diems.id_sinhvien','diems.diem_tong_sv','diems.diem_tong_gv','diems.diem_tong_hd','sinhviens.ten as ten_sv','sinhviens.ho as ho_sv','sinhviens.ma_sv as ma_sv','lops.ten_lop as ten_lop','hockys.hk_nk as hocky','lops.id_lop as id_lop','hockys.id_hocky as id_hk')->distinct()
     ...
     //    return dd($diem[0]);
        try{
         for($i = 0; $i < count($diem_update); $i++){
             $diem_update[$i]->diem_hd = $diem[$i];
             $diem_update[$i]->save();
         }
         $this->update_diem_tong_hd($request);
    ...
    //     }
        ...
        // diem tieu chi hd
    public function get_diem_hd_tieuchi1($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diemtc_1 = Diem::with('noidung')->where('id_sinhvien', $id_sinhvien)->where('id_hocky',$id_hocky)->whereRelation('noidung','id_tc',1)->sum('diem_hd');
        if($diemtc_1 >= 20){
            return $diemtc_1 = 20;
        }
        return $diemtc_1;
    }
    public function get_diem_hd_tieuchi2($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diemtc_1 = Diem::with('noidung')->where('id_sinhvien', $id_sinhvien)->where('id_hocky',$id_hocky)->whereRelation('noidung','id_tc',2)->sum('diem_hd');
        if($diemtc_1 >= 25){
            return $diemtc_1 =25;
        }
        return $diemtc_1;
    ...
    public function get_diem_hd_tieuchi3($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diemtc_1 = Diem::with('noidung')->where('id_sinhvien', $id_sinhvien)->where('id_hocky',$id_hocky)->whereRelation('noidung','id_tc',3)->sum('diem_hd');
        if($diemtc_1 >= 20){
            return $diemtc_1 = 20;
        }
        return $diemtc_1;
    }
    public function get_diem_hd_tieuchi4($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diemtc_1 = Diem::with('noidung')->where('id_sinhvien', $id_sinhvien)->where('id_hocky',$id_hocky)->whereRelation('noidung','id_tc',4)->sum('diem_hd');
        if($diemtc_1 >= 20){
            return $diemtc_1 = 20;
        }
        return $diemtc_1;
    }
    public function get_diem_hd_tieuchi5($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diemtc_1 = Diem::with('noidung')->where('id_sinhvien', $id_sinhvien)->where('id_hocky',$id_hocky)->whereRelation('noidung','id_tc',5)->sum('diem_hd');
        if($diemtc_1 >= 10){
            return $diemtc_1 =10;
        }
        return $diemtc_1;
    }
    //tinh diem tong HD tu diem GV
    public function update_diem_tong_hd($request){
        $id_sinhvien = $request->id;
        $id_hocky = $request->hocky;
        $diem_1 = $this->get_diem_hd_tieuchi1($request);
        $diem_2 = $this->get_diem_hd_tieuchi2($request);
        $diem_3 = $this->get_diem_hd_tieuchi3($request);
        $diem_4 = $this->get_diem_hd_tieuchi4($request);
        $diem_5 = $this->get_diem_hd_tieuchi5($request);
    
       $diem_tong_hd = $diem_1 + $diem_2 + $diem_3 + $diem_4 + $diem_5;
        // return dd($diem_tong_hd);
       $diem_update = Diem::where('id_sinhvien', $id_sinhvien)->where('id_hocky', $id_hocky)->get();
    
        $diem_tong_gv_arr = array();
            for($i = 0; $i < count($diem_update); $i++)
            {
            $diem_tong_gv_arr[$i] = "$diem_tong_hd";
            $diem_update[$i]->diem_tong_hd = $diem_tong_gv_arr[$i];
                $diem_update[$i]->save();
            }
        }
